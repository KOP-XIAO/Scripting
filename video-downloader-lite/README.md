...
| 抖音分享链接 | ✅ | H5 `_ROUTER_DATA` 解析无水印直链，零配置（移植自 SHY-downloader） |
| 直播吧等内嵌腾讯云点播的网页 | ✅ | 页面嗅探（vod-player → getplayinfo 直链），零配置 |
| 普通网页（og:video / 内嵌 video_url / 裸直链） | ✅ | 页面嗅探，零配置 |
| YouTube / B站 / X / IG 等平台链接 | ⚙️ | 嗅探未命中时才走 cobalt 解析实例 |
